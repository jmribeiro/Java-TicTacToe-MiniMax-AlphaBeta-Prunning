...
public class Board {
	...
	private void fillCoordinates(int line, int collumn, char piece) throws InvalidCoordinatesException, InvalidPieceException, PositionOccupiedException{
		if(!Utilities.validCoordinates(line, collumn)){
			throw new InvalidCoordinatesException(line, collumn);
		}else if(!Utilities.validPiece(piece)){
			throw new InvalidPieceException(piece);
		}else if(!isEmpty(line, collumn)){
			throw new PositionOccupiedException(line, collumn);
		}else{
